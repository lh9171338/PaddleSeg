...
class SegFormer(Segmentor):
    ...
    def forward(self, x):
        ...
        _c4 = (
            self.linear_c4(c4)
            .transpose([0, 2, 1])
            .reshape([0, 0, c4_shape[2], c4_shape[3]])
        )
        _c4 = F.interpolate(
            _c4,
            size=c1_shape[2:],
        ...
        _c3 = (
            self.linear_c3(c3)
            .transpose([0, 2, 1])
            .reshape([0, 0, c3_shape[2], c3_shape[3]])
        )
        _c3 = F.interpolate(
            _c3,
            size=c1_shape[2:],
        ...
        _c2 = (
            self.linear_c2(c2)
            .transpose([0, 2, 1])
            .reshape([0, 0, c2_shape[2], c2_shape[3]])
        )
        _c2 = F.interpolate(
            _c2,
            size=c1_shape[2:],
        ...
        _c1 = (
            self.linear_c1(c1)
            .transpose([0, 2, 1])
            .reshape([0, 0, c1_shape[2], c1_shape[3]])
        )
...
if __name__ == "__main__":
    from ppseg.backbones import MixVisionTransformer_B0

    backbone = MixVisionTransformer_B0(
