...
def download_file_and_uncompress(
    ...
    extraname = (
        savename if extraname is None else os.path.join(extrapath, extraname)
    )
    ...
    full_path = (
        os.path.join(extraname, filename)
    ...
    if not os.path.exists(
        full_path
    ):  # If pretrained model exists, skip download process.
        ...
        if rank_id_curr_node == 0:
            if not os.path.exists(savename):
                ...
                if (not tarfile.is_tarfile(savepath)) and (
                    ...
                    savename = _uncompress_file(
                        savepath, extrapath, delete_file, print_progress
                    )
                    savename = os.path.join(extrapath, savename)
